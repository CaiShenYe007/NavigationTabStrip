...
public class NavigationTabStrip extends View implements ViewPager.OnPageChangeListener {
    ...
    private float mStripLeftRightPadding;
    ...
    public NavigationTabStrip(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        try {
            ...
            setStripHorizantalPadding(
                    typedArray.getDimension(
                            R.styleable.NavigationTabStrip_nts_strip_left_right_padding, 0));
    ...
    public void setStripHorizantalPadding(float value) {
        mStripLeftRightPadding = value;
        postInvalidate();
    }
    ...
    protected void onDraw(final Canvas canvas) {
        float left = mStripLeft - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F);
        float right = mStripRight - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F);
        if (mStripType != StripType.POINT && mStripLeftRightPadding < (mTabSize / 2)) {
            left += mStripLeftRightPadding;
            right -= mStripLeftRightPadding;
        }
